...
- 第4部 4.4.1 で、陰陽を「属性を落とした二状態」として扱うための歴史的補助線として参照する。
- これは記号・数理としての陰陽／易の話であり、史料に残る陰陽師・陰陽道、伝承上の陰陽師像、現代文化の陰陽師イメージとは分けて扱う。
...
- 第4部 4.4 で、史料に残る陰陽師・陰陽道、伝承・物語としての陰陽師、現代文化の中の陰陽師イメージを分ける。
- 第4部 4.5 で、デジタル技術と比較しているのは職能や宗教的意味ではなく、状態を読み、記号化し、作法化し、代理的な働きを通じて現実へ作用させる問題の型だと限定する。
- 史料・伝承・現代文化の層に含まれる宗教的、儀礼的、呪術的、スピリチュアル、娯楽的な要素や、易経とは直接関係しない思想・実践を、デジタル技術の説明へ直接混ぜないための安全装置として使う。

主張:

- 陰陽道は、古代日本で中国から伝わった陰陽五行説、易、道教などの知識をもとに形成され、陰陽寮で管理され、暦や天文に関する書物や道具とともに研究されたと説明される。
- 陰陽師は、暦、天文、方位、占い、祭祀、呪符などの実践に関わったと説明される。
- 安倍晴明は実在した陰陽師だが、陰陽道の浸透とともに伝奇的なイメージが付け加わり、説話や物語の層が形成されたと説明される。
- 近世以降、陰陽道を起源とするまじないや占いが書物化され、庶民にも広がったと説明される。
- したがって、史料層、伝承層、現代文化層を、易や二進法の記号・数理層と混ぜて扱わない。

ソース:

- [国立歴史民俗博物館「陰陽師とは何者か－うらない、まじない、こよみをつくる－」](https://www.rekihaku.ac.jp/news/20230706.html)
- [国立歴史民俗博物館 企画展示ページ「陰陽師とは何者か」](https://www.rekihaku.ac.jp/event/2023_exhibitions_kikaku_on.html)
- [國學院大學博物館「土御門家がみた宇宙－江戸時代の天文観測」](https://museum.kokugakuin.ac.jp/special_exhibition/detail/2023_tsuchimikado.html)

確度:

- 高: 陰陽師・陰陽道に、暦、天文、方位、占い、祭祀、呪符などの史料上確認できる実践があること。
- 高: 安倍晴明の実在と、後代の伝奇的イメージを分けて扱う必要があること。
- 高: 史料層、伝承層、現代文化層を、易・二進法の記号的対応と混同しない方針。
- 中: これらをデジタル技術の「問題の型」と比較すること。これは本文側の仮説であり、歴史的同一性ではない。
- 低: 陰陽道、伝承、現代文化の宗教的・呪術的要素を、現代デジタル技術の直接の根拠や原型とする主張。本文では採用しない。

## 日本語の気と制御像

本文での使い方:

- 第4部 4.6.4 で、陰陽や符よりも日常語に近い制御感覚として、「気を遣う」「気を配る」「気を回す」「気が合う」「気配を読む」を扱う。
...
- 第4部 4.6.1 で、`code`、`符`、`符号`、`符文` を、語源の同一視ではなく、しるし、規則体系、形式、照合、実行条件の構造的対応として扱う。
...
- 第4部 4.4.4 で、映画『陰陽師』を、陰陽道史の根拠ではなく、現代日本語話者に共有されたポップカルチャー上の認知インターフェースとして扱う。
...
- 第4部 4.7 で、魔法、魔人（ジン）、符、呪文、陰陽、式神を、技術の本質ではなく、状態、操作、接続、伝搬、制御の構造を見えるようにする補助線として扱う。
- 第7部 7.3 で、魔法という比喩を、複雑な内部機構や因果連鎖が利用者には見えにくく、結果だけが自然に現れる状態として説明する。
- `application` を「用途へ当てる」「実用に供する」方向で読む案は、本文の中心にはまだ入れず、保留中の補助線として扱う。

主張:

- 比喩は、対象そのものではなく、見えにくい構造を説明するための入口である。
- 魔法という比喩は、技術が本当に超自然的であることではなく、利用者から内部機構が見えにくいにもかかわらず、望ましい状態変化が起きることを説明するために使う。
- Mark Weiser の ubiquitous computing 論は、計算機が環境へ溶け込み、利用者がその存在を意識しなくなる方向の補助線として使える。これは、裏側の仕組みが消えるという意味ではなく、利用者の前景から退くという意味で読む。
- `application` には、何かを使用すること、用途、特定の仕事を行うプログラム、また「接触させる」「特定目的へ用いる」方向の語史がある。これは、汎用的な機能を特定の目的やユーザーの望みへ当てるという説明の補助線になりうる。

ソース:

- [Scientific American: Mark Weiser “The Computer for the 21st Century”](https://www.scientificamerican.com/article/the-computer-for-the-21st-century/)
- [Merriam-Webster: “application”](https://www.merriam-webster.com/dictionary/application)
- [Online Etymology Dictionary: “application”](https://www.etymonline.com/word/application)

ローカル文書:

- `00_fragments/md_files/single_threads/2026-05-12T11-02-08_プログラミング_-_GitHub文書の閲覧確認.md`
- `01_curated/single_threads_review_integration_20260512.md`

確度:

- 高: 比喩を本質ではなく、構造を説明する補助線として扱う方針。
- 中: Weiser を、技術が日常環境へ溶け込み、利用者から見えにくくなる補助線として使うこと。
- 中: `application` を、汎用的な機能を特定用途へ当てる語感として補助的に使うこと。
- 低: 魔法という比喩を、技術の本質そのものとして扱う主張。本文では採用しない。
- 要確認: Arthur C. Clarke の「十分に進んだ技術は魔法と見分けがつかない」という言い方を本文に入れる場合は、一次に近い出典確認後に扱う。

## スマートデバイス需要と魔法・魔人（ジン）の提示
...
- `sword-voice-agent` は、刀印ジェスチャー、音声、AI エージェント、家電制御、投影表示を束ねる、ランプの魔人（ジン）型インターフェースの試作として扱える。
